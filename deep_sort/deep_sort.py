...
class DeepSort(object):
    ...
    def init(cls,
             ...
             n_init=3,
             nn_budget=100):
        if model_config == cls.CACHED_CONFIG and cls.CACHED_EXTRACTOR is not None:
            extractor = cls.CACHED_EXTRACTOR
            # given_extrator = cache
            return DeepSort(model_config,
                            device,
                            max_dist,
                            max_iou_distance,
                            max_age,
                            n_init,
                            nn_budget,
                            given_extrator=extractor)
        else:
            # given_extrator = None
            model = DeepSort(model_config,
                             device,
                             max_dist,
                             max_iou_distance,
                             max_age,
                             ...
                             nn_budget,
                             given_extrator=None)
            cls.CACHED_EXTRACTOR = model.extractor
            cls.CACHED_CONFIG = model_config
            return model
    ...
    def __init__(self,
                 ...
                 nn_budget=100,
                 given_extrator=None):
        if given_extrator is not None:
            self.extractor = given_extrator
        else:
            ...
            if is_model_in_factory(model):
                ...
                if not file_exists(model_path):
                    gdown.download(get_model_link(model),
                                   model_path,
                                   quiet=False)
